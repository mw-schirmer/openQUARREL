...
```{r calibration_results, eval=FALSE}
...
Simulate model, similar to `vignette("include_snow_module")`, but taking now using calibrated parameters.
...
For validation there are two plots availalbe which can be right now only only saved to disc as pdf 
...
For example `mae__power__-0.5` is the mean absolute error calculated with the 
[hydroGOF::mae](https://rdrr.io/cran/hydroGOF/man/mae.html), and using a power 
transformation with exponent 0.2.
...
Show the results which is a tibble:
```{r show_table}
...
a different calibration function and streamflow transformation for calibration 
